  ...
  const fetchFormData = async () => {
    try {
      ...
      const [ttRes, divRes, subRes, rmRes, lrRes, taughtRes, facRes, subProxyRes] = await Promise.all([
        ...
        supabase.from('substitutions').select('*, absent_faculty:absent_faculty_id(id, full_name, initials), timetable:timetable_id!substitutions_timetable_id_fkey(*, subjects(*), divisions(*), rooms(*), time_slots(*))').eq('proxy_faculty_id', profile.id).eq('substitution_date', t).eq('status', 'active')
      ])
