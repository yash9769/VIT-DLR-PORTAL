...
const TimetableCard = ({ entry, submitted, onSubmit }) => (
  <div className="glass-card p-4 flex items-center gap-4 transition-all active:scale-98">
    ...
    <div className="w-14 flex-shrink-0 text-center">
      ...
      <div className="w-0.5 h-3 mx-auto my-1 rounded-full" style={{ background: 'rgba(74,108,247,0.3)' }} />
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center gap-2 mt-1 flex-wrap">
        <span className="text-xs px-2 py-0.5 rounded-md font-medium" style={{ background: 'rgba(74,108,247,0.12)', color: '#7090ff' }}>
...
export default function FacultyDashboard() {
  ...
  return (
    <div className="px-4 pt-5 pb-4 space-y-5 animate-fade-in">
      {/* Greeting */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: 'rgba(74,108,247,0.8)' }}>
      ...
      {/* Pending alert */}
      {stats.pending > 0 && (
        <button onClick={() => navigate('/faculty/submit')} className="w-full flex items-center gap-3 p-4 rounded-2xl transition-all active:scale-98" style={{ background: 'rgba(74,108,247,0.12)', border: '1px solid rgba(74,108,247,0.3)' }}>
        ...
        <div>
          ...
          <div className="space-y-2">
            {yesterdayRecords.filter(r => r.approval_status === 'pending').map(r => (
              <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(210,153,34,0.08)', border: '1px solid rgba(210,153,34,0.2)' }}>
